...
<!-- ### **Map to files**{data-width=200} -->


<!-- ```{r lib_table} -->

<!-- lib<-subset(lib, select=c("Z", "SYMAM", "MAT", "MAP")) -->
<!-- libo<-lib -->
...
lib2 <-data.table::fread(paste0(gpath,'JEFF33_vs_JEFF32.csv'), header = TRUE, sep = ",", stringsAsFactors = TRUE) %>% # RELATIVE PATH
  ...
  merge(mats) # MERGE TO HAVE MASS, CHARGE, ETC

lib2$SYMAM<-paste0(lib2$X,"-",lib2$A,"-",lib2$M)

lib2<-subset(lib2, select=c('Z', 'SYMAM', 'NEW'))

lib3<-merge(x=lib, y=lib2, all.x=TRUE) #, by="SYMAM")
...
<!-- df <-data.table::fread('csv/JEFF33_vs_JEFF32.csv', header = TRUE, sep = ",", stringsAsFactors = TRUE) %>% # RELATIVE PATH -->
<!--   subset(MT!=451) %>%  # REMOVE MF1MT451 -->
<!--   dplyr::group_by(MAT) %>%  -->
<!--   dplyr::summarize(NEW=all(VERORIG==3.3)) %>% -->
<!--   subset(NEW==TRUE) %>% # SET TO FALSE TO HAVE MATS THAT HAVE NOT CHANGED -->
