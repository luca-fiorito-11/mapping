...
comp <- merge(lib1, lib2, by = c('MFDES', 'MTDES', 'MF', 'MT'), all=TRUE) %>%
  data.table::setnames(old=c("LIBVERORIG.x","LIBVERORIG.y"), new=c("LIB1","LIB2")) %>%
  data.table::setnames(old=c("MFDES","MTDES"), new=c("MF TYPE","MT TYPE")) %>%
...
Origin of **updated** sections in `r mLIBVER` file,  with respect to reference : `r mLIBVER2`  

```{r table_updated} 
comp %>% subset(LIB1!=LIB2 & LIB1!="NA" & LIB2!="NA") %>% # diff occurrences
  data.table::setnames(old=c("LIB1", "LIB2"), new=c(mLIBVER, mLIBVER2)) %>%
...
Origin of **new** sections in `r mLIBVER` file,  with respect to reference : `r mLIBVER2`  
...
Origin of **unchanged** sections in `r mLIBVER` file, with respect to reference : `r mLIBVER2` 

```{r table_same} 
comp %>% subset(LIB1==LIB2, select = c('MF TYPE', 'MT TYPE', 'MF', 'MT', 'LIB1')) %>% # same occurrences
  data.table::setnames(old=c("LIB1"), new=c("Origin")) %>%
  datatable(filter = 'top',
            rownames = F,
            options = list(
              pageLength=10,
...
Origin of **erased** sections in `r mLIBVER` file that were present in reference : `r mLIBVER2` 
